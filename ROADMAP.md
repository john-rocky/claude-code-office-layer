...
- ✅ **PII detector** — `safety/pii.py::scan(text)` with three pain-ranked buckets: (1) `mynumber` keyword-anchored 12-digit (PPC-reportable), (2) `credit_card` Luhn + context blacklist (`INV` / `伝票` / `追跡` / `JAN` / `ISBN` / `978` / `979` / `法人番号` / `商品コード` / `口座`), (3) `phone` JP shapes with ≥10-digit floor. `email_draft.build_email_draft_body` scans the assembled body (excluding front-matter) and promotes each hit to a `Verify <type> at <evidence>` checklist row; `draft_email_from_evidence` returns `pii_hit_count` + `pii_hits[]`; MCP audit log records `pii_hit_count`. 25 new tests (21 unit covering each guard + 4 integration in `test_email_draft.py`).
- ✅ **Mass-operation guard** — `safety/risk.py::BULK_MODIFY_THRESHOLD = 5` with `bulk_modify` demoted out of `HIGH_OPS` into a special-cased MEDIUM op that flips `requires_confirmation=True` when `row_count > threshold`. `safety/pretool.intercept` gains `row_count` + `confirm` kwargs and a parallel `confirmation` field on `InterceptResult` (refusal still owns HIGH; confirmation owns the new soft block). `extract_invoices_to_table` calls the gate AFTER `build_invoice_rows` (so the row count is known), stages a `<stem>.preview.csv` (head 10, non-timestamped) on first call, returns `confirmation_required: true` + `preview_path`; second call with `confirm=true` (MCP) / `--confirm` (CLI) writes the timestamped full CSV. Outside-workspace / read-only escalations still refuse uniformly — `confirm=true` deliberately does NOT cross those hard rules. 15 new tests (8 in `test_pretool_intercept.py` covering threshold edge cases, no-row-count safety, refuse-still-wins-over-confirm; 7 in `test_invoices_table.py` covering at-threshold writes, over-threshold preview, with-confirm full write, head cap at 10, preview overwrite on rerun, read-only refuses before preview, `confirmed` reflection). Verified live on `ws_91d1629c00ed` (probe fixture, 6 rows): no-confirm stages `/tmp/office_layer_pii_probe/export_test.preview.csv` + returns `confirmation_required`; `--confirm` writes `export_test-YYYYMMDD-HHMMSS.csv`.
...
- **2026-05-18** Phase 3 mass-operation guard shipped on `extract_invoices_to_table`. Pulled `bulk_modify` out of `HIGH_OPS` and special-cased it in `classify_operation`: stays MEDIUM (refusing the natural batch path would block the only legitimate use case this tool exists for — there is no version of "you have too many invoices" that should be a hard refuse), but flips `requires_confirmation=True` whenever the caller supplies `row_count > BULK_MODIFY_THRESHOLD` (= 5). Workspace-policy / outside-workspace checks still escalate to HIGH for the usual reasons, and HIGH still wins over `confirm=True` — the soft gate exists to slow down a noisy export, not to bypass the hard workspace boundary. `safety/pretool.InterceptResult` grows a parallel `confirmation` field (alongside the existing `refusal`) and `intercept` gains `row_count` + `confirm` kwargs; the confirmation dict mirrors the refusal shape (`{confirmation_required, reason, row_count, threshold, how_to_proceed, risk}`) so callers can render either uniformly. `extract_invoices_to_table` calls the gate AFTER `build_invoice_rows` — putting it before would mean refusing without knowing what would have been written, which is the *opposite* of helpful — and stages a `<output_stem>.preview.csv` (first 10 rows + the same 12-column header) on first call. Preview filename is deliberately NOT timestamped: it's a transient review artifact that should overwrite itself on re-runs, unlike the real export. Threshold of 5 picked from the user prompt's framing ("> 5 file writes → require explicit yes-to-all") — small enough to catch a meaningful batch but high enough that the canonical small-team sample workspace (4-6 invoices in `examples/sample_workspace/invoices/`) is right at the boundary and exercises the gate naturally. MCP audit log records `confirmed` + `confirmation_required` so operator dashboards can distinguish "fired and accepted" from "fired and bailed". 15 new tests (8 pure intercept + 7 workflow integration): under/at-threshold writes directly, over-threshold stages preview + blocks full write, with-confirm proceeds, 50-row export still caps preview at 10, preview path stable across re-runs, read-only refusal precedes preview, no-row-count call doesn't accidentally escalate (the old `HIGH_OPS` entry would have refused unconditionally; the test pins the new contract). 194 passing total (was 179 + 15). Live verified on probe fixture (`ws_91d1629c00ed`, 6 rows): bare call → preview + confirmation_required; `--confirm` → `export_test-YYYYMMDD-HHMMSS.csv` with all 6 rows.
- **2026-05-18** Phase 3 PII detector shipped as `safety/pii.py::scan(text) -> list[PIIHit]`, wired into `workflows/email_draft.build_email_draft_body`. Priority ranking is by **leak pain × FP cost**, not by detection difficulty: (1) `mynumber` first because a JP個人番号 leaving the company is PPC-reportable and 12-digit runs are rare enough that a keyword anchor (`マイナンバー` / `個人番号`) costs almost no recall while killing every realistic false positive (法人番号 is 13 digits and explicitly excluded as a negative context token); (2) `credit_card` second because financial leak is real but the false-positive surface is huge — invoices routinely carry 13-16 digit codes (JAN, ISBN-13, 法人番号, 追跡, bank account) that naïve regexes flag constantly, so Luhn + a context blacklist (`INV` / `伝票` / `追跡` / `JAN` / `ISBN` / `978` / `979` / `法人番号` / `商品コード` / `口座` / `普通` / `当座` within 20 chars upstream or 8 chars downstream) is mandatory to make the checklist row trustworthy; (3) `phone` third because while leaks are common in B2B context they are low-impact, and JP-shape regex + ≥10 digit floor cleanly separates real phones from postal codes (7 digits, the canonical FP). Ranking decided after manually running `office-layer email draft` against a writable copy of the sample workspace + a probe fixture (`/tmp/office_layer_pii_probe/invoices/invoice-pii-probe-2026-04.md`) that bundled all 8 realistic FP shapes (invoice ID with separators, tracking number, JAN, ISBN, 法人番号, bank account, postal code, FAX). The scan runs over the **assembled body** (citation previews + extracted-field checklist + extra_context), not over the raw `extracted_text` of every source — the contract is "what is about to be staged into a file the user will paste into Gmail", so the scan boundary matches the artifact boundary; broader scope would just noise the checklist with PII that lives in the workspace index but never makes it into the outgoing email. Front-matter is excluded from the scan because workspace IDs / draft IDs / paths would otherwise be a low-grade noise source (none pass Luhn today but cheaper to scope than to defend). Hits are promoted to a dedicated `**PII 検出**` / `**PII detected**` section with one row per hit including a 12-char evidence window — diffable order (sorted by `(start, kind)`) so regenerating the draft produces the same checklist. `draft_email_from_evidence` returns `pii_hit_count` + `pii_hits[]`; MCP audit log records `pii_hit_count`. No new safety _rules_ in the refuse sense — the PII section is purely advisory checklist (the email is still staged) because false-positive cost on a strict block would be much higher than the leak risk we are guarding against. 25 new tests (21 unit in `tests/test_pii.py` per guard + 4 integration in `tests/test_email_draft.py`), 179 passing total (was 154 + 25).
